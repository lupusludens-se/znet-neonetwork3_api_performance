...
@Component({
  selector: 'neo-battery-storage',
  templateUrl: './battery-storage.component.html',
...
export class BatteryStorageComponent implements OnInit {
  ...
  form: FormGroup = this.formBuilder.group({
    minimumAnnualPeakKW: ['', [Validators.required, Validators.min(1), Validators.max(INT32_MAX)]],
    contractStructures: ['', Validators.required],
    minimumTermLength: ['', [Validators.min(1), Validators.max(INT32_MAX)]],
    valuesProvided: [null, Validators.required],
  ...
  ngOnInit(): void {
    if (this.addProjectService.currentFlowData?.projectDetails) {
      const formVal: ProjectBatteryStorageDetailsInterface = this.addProjectService.currentFlowData
        ?.projectDetails as ProjectBatteryStorageDetailsInterface;

      this.form.patchValue({
        minimumAnnualPeakKW: formVal.minimumAnnualPeakKW,
  ...
  chooseCheckboxControlVal(val: Partial<TagInterface>, unselect: boolean, controlName: string) {
  ...
  changeStep(step: number): void {
    this.form.controls['minimumAnnualPeakKW'].addValidators(Validators.required);
  ...
  saveDraft(): void {
    this.form.controls['minimumAnnualPeakKW'].removeValidators(Validators.required);
  ...
  updateFormValidity(): void {
    this.form.controls['minimumAnnualPeakKW'].updateValueAndValidity();
