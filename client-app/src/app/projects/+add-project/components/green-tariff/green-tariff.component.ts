...
@Component({
  selector: 'neo-green-tariff',
  templateUrl: './green-tariff.component.html',
  styleUrls: ['../../add-project.component.scss']
})
export class GreenTariffComponent implements OnInit {
  stepsList = AddProjectStepsEnum;
  projectTypes = ProjectTypesSteps;
  form: FormGroup = this.formBuilder.group({
    utilityName: ['', [CustomValidator.required, Validators.maxLength(100)]],
    programWebsite: ['', [Validators.required, CustomValidator.url, Validators.maxLength(2048)]],
    minimumPurchaseVolume: ['', [Validators.required, Validators.min(1), Validators.max(INT32_MAX)]],
    termLengthId: ['', Validators.required],
  ...
  ngOnInit(): void {
    if (this.addProjectService.currentFlowData?.projectDetails) {
      const formVal: ProjectGreenTariffDetailsInterface = this.addProjectService.currentFlowData
        ?.projectDetails as ProjectGreenTariffDetailsInterface;

      this.form.patchValue({
        utilityName: formVal.utilityName,
        programWebsite: formVal.programWebsite,
        minimumPurchaseVolume: formVal.minimumPurchaseVolume,
        termLengthId: formVal.termLengthId,
  ...
  changeStep(step: number): void {
    this.form.controls['utilityName'].addValidators(CustomValidator.required);
    this.form.controls['programWebsite'].addValidators(Validators.required);
    this.form.controls['minimumPurchaseVolume'].addValidators(Validators.required);
    this.form.controls['termLengthId'].addValidators(Validators.required);
  ...
  saveDraft(): void {
    this.form.controls['utilityName'].removeValidators(CustomValidator.required);
    this.form.controls['programWebsite'].removeValidators(Validators.required);
    this.form.controls['minimumPurchaseVolume'].removeValidators(Validators.required);
    this.form.controls['termLengthId'].removeValidators(Validators.required);
  ...
  updateFormValidity(): void {
    this.form.controls['utilityName'].updateValueAndValidity();
    this.form.controls['programWebsite'].updateValueAndValidity();
    this.form.controls['minimumPurchaseVolume'].updateValueAndValidity();
    this.form.controls['termLengthId'].updateValueAndValidity();
