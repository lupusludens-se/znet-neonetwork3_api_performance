...
export class ToolManagementPermissionGuard implements CanLoad {
  ...
  canLoad(): boolean {
    ...
    if (!currentUser) {
      ...
      hasPermission = this.permissionService.hasPermission(allPermissions, PermissionTypeEnum.ToolManagement);
    } else {
      hasPermission = this.permissionService.userHasPermission(currentUser, PermissionTypeEnum.ToolManagement);
    }
