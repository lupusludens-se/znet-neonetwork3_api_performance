...
export class ProjectViewPermissionGuard implements CanActivate {
  ...
  canActivate(): boolean {
    ...
    if (!currentUser) {
      ...
      hasPermission = this.permissionService.hasPermission(allPermissions, PermissionTypeEnum.ProjectCatalogView);
    } else {
      hasPermission = this.permissionService.userHasPermission(currentUser, PermissionTypeEnum.ProjectCatalogView);
    }
