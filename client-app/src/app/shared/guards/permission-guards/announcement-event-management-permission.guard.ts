...
export class AnnouncementManagementPermissionGuard implements CanLoad {
  ...
  canLoad(): boolean {
    ...
    if (!currentUser) {
      ...
      hasPermission = this.permissionService.hasPermission(allPermissions, PermissionTypeEnum.AnnouncementManagement);
    } else {
      hasPermission = this.permissionService.userHasPermission(currentUser, PermissionTypeEnum.AnnouncementManagement);
    }
