...
export class CompanyManagementPermissionGuard implements CanLoad {
  ...
  canLoad(): boolean {
    ...
    if (!currentUser) {
      ...
      hasPermission = this.permissionService.hasPermission(allPermissions, PermissionTypeEnum.CompanyManagement);
    } else {
      hasPermission = this.permissionService.userHasPermission(currentUser, PermissionTypeEnum.CompanyManagement);
    }
