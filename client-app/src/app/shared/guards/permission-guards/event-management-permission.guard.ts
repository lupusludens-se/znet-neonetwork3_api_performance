...
export class EventManagementPermissionGuard implements CanLoad {
  ...
  canLoad(): boolean {
    ...
    if (!currentUser) {
      ...
      hasPermission = this.permissionService.hasPermission(allPermissions, PermissionTypeEnum.EventManagement);
    } else {
      hasPermission = this.permissionService.userHasPermission(currentUser, PermissionTypeEnum.EventManagement);
    }
