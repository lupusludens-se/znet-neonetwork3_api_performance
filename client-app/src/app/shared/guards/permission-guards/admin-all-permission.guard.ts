...
export class AdminAllPermissionGuard implements CanActivate {
  ...
  canActivate(): boolean {
    ...
    if (!currentUser) {
      ...
      hasPermission = this.permissionService.hasPermission(allPermissions, PermissionTypeEnum.AdminAll);
    } else {
      hasPermission = this.permissionService.userHasPermission(currentUser, PermissionTypeEnum.AdminAll);
    }
