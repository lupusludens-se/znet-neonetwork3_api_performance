...
{
    internal class CompanyProjectNotificationHandler : BaseNotificationHandler
    {
        public CompanyProjectNotificationHandler(ApplicationContext context, IMapper mapper, NotificationType type)
        ...
        public override async Task<int> AddAsync(int userId, ISingleNotificationDetails details)
        {
            if (details is CompanyNotificationDetails)
            {
                ...
                return await InsertOrUpdateNotification(new NotificationDTO(userId, _type, details),
                    (currentDetails, _) =>
                    {
                        return new GroupCompanyProjectNotificationDetails
                        {
